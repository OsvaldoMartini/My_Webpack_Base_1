...
FacetsVM.prototype.applyFacets = function() {
  ...
  for (var i = 0; i < self.selectedItems().length; i++) {
    ...
    switch (f.queryKey) {
      ...
      default:
        self.applyFacet(f.indexer + exclude, f.value, overridden, facets, true);
        break;
    }
...
FacetsVM.prototype.applyFacet = function(
  ...
  self.addFacetQuery(indexer, value, overridden, facets);
...
FacetsVM.prototype.addQuery = function(
  ...
  self.addFacetQuery(indexer, value, overridden, search, true);
...
FacetsVM.prototype.addFacetQuery = function(
  ...
  if (facets.hasOwnProperty(indexer)) {
    ...
    facets[indexer] = { operator: "<and>", values: [value] };
