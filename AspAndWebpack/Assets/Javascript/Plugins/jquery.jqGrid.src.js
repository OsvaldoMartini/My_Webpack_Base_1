...
$.extend($.jgrid,{
	...
	getCellIndex : function (cell) {
		...
		if (bowser.msie) { return $.inArray(c, c.parentNode.cells); }
		return c.cellIndex;
	},
	...
	return this.each( function() {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			while (i<len) {
			    ...
			    if (ts.p.thumbnailView === true) {
			        if (colCount === 1 || colCount === ts.p.thubmnailColumns) {
			            if (colCount == 1) {
			                rowData[iStartTrTag] = constructTr(idr, hiderow, cn1, rd, cur, selr);
			            }			            
			            if (colCount === ts.p.thubmnailColumns) {
			                rowData.push("</tr>");
			                colCount = 0;
			            }
			        }
		...
		$("th",thr).each(function ( j ) {
			...
			if(ts.p.colModel[j].resizable){
				...
				if(!bowser.opera) { $(res).css("cursor","col-resize"); }
		...
		$(grid.bDiv)
			.append($('<div style="position:relative;'+(isMSIE && bowser.version < 8 ? "height:0.01%;" : "")+'"></div>').append('<div></div>').append(this))
...
params:{}};
var s=0,H=$.jqm.hash,A=[],ie6=bowser.msie&&(bowser.version == "6.0"),F=false,
...
"use strict";	
	...
	$.fn.fmatter.checkbox =function(cval, opts) {
	    ...
	    //We really should not be editting this 3rd party plugin i.e. changeset 7043!!! Considering there is only one day before code freeze, 
	    //and I have no time to extract that changeset's logic I have had to add logic here to fix a bug DE30673 in time for release. WE MUST EXTRACT THIS LOGIC AND THE 7043 CHANGESET!!!!!!
        //Delete the following 'IF' statement once extracted - G.H
		if (!isNaN(opts.rowId)){
            return "<input type=\"checkbox\" " +bchk + " value=\"" +cval+ "\" offval=\"no\" " +ds+ " data-itemid=\"" + opts.rowId + "\"/>";
		}
				...
				$('td[role="gridcell"]',ind).each( function(i) {
					...
					if ( nm != 'cb' && nm != 'subgrid' && nm != 'rn') {
						...
						if(cm[i].editable===true) {
							...
							$.jgrid.bindEv( elc, opt, $t);
							//Again IE
							if(cm[i].edittype == "select" && cm[i].editoptions!==undefined && cm[i].editoptions.multiple===true  && cm[i].editoptions.dataUrl===undefined && bowser.msie) {
								$(elc).width($(elc).width());
							}
							cnt++;
						}
...
setSubGrid : function () {
	return this.each(function (){
		...
		$t.p.colModel.unshift({name:'subgrid',width: (bowser.safari || bowser.webkit) ?  $t.p.subGridWidth+$t.p.cellLayout : $t.p.subGridWidth,sortable: false,resizable:false,hidedlg:true,search:false,fixed:true});
		cm = $t.p.subGridModel;
		if(cm[0]) {
