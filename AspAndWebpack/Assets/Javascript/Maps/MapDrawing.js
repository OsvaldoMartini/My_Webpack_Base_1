...
// Needd to update grpahic which areas are being search on edit
    ...
    self.search = function(area, areaToRemove){
        if(area === areaToRemove){ areaToRemove = null; }
        ...
        if(area){
            _mapPageVM.facetsVM.applyFacet(indexer, area, true, undefined, true);
        }else{
    ...
    self.selectArea = function(){
        showCustomPopup({
            ...
            "<div class='grid-1-3' style='overflow-y: auto;max-height: 440px;'>"+
                "<ul class='nav nav-tabs nav-stacked' data-bind='foreach: existingAreas, visible:!isLoadingAreas()'>"+
                    "<li data-bind='css:{\"active\":$parent.viewingArea() && $parent.viewingArea().id===$data.id}'>"+
                        "<a href='' data-bind='click: $parent.viewingArea.bind($parent, $data)'>"+
                            "<span data-bind='text: name;'></span>" +
                            "<i class='icon-right-open pull-right' data-bind='visible:$parent.viewingArea() && $parent.viewingArea().id===$data.id' ></i>" +
                        "</a>"+
                    "</li>"+
                "</ul>"+
                ...
                "<ul class='nav nav-tabs nav-stacked' data-bind='foreach: definedAreaFiltered, visible:definedAreaFiltered().length'>"+
                    "<li data-bind='css:{\"active\":$parent.viewingArea() && $parent.viewingArea().id===$data.id}'>"+
                        "<a href='' data-bind='click: $parent.viewingArea.bind($parent, $data),event:{keydown:$parent.drawAndSearchAreaKey.bind($parent)}'>"+
                            "<span data-bind='text: name;'></span>" +
                            "<i class='icon-right-open pull-right' data-bind='visible:$parent.viewingArea() && $parent.viewingArea().id===$data.id' ></i>" +
                        "</a>"+
                    "</li>"+
                ...
                "</div>"+
            "</div>"+
            "<div class='loading text' data-bind='visible:isLoadingAreas'>Loading Areas</div>",
            modalOptions: {
                backdrop: false
            },
            viewModel: self,
