...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
			...
			<xsl:if test="$showTitle = 'true'">
				<h1 class="page-header u-margin-Bm u-margin-Ts u-padding-Bs">
						<xsl:if test="janes:record/@updated = 'y'">
							<xsl:attribute name='class'>page-header u-margin-Bm u-margin-Ts u-padding-Bs green1</xsl:attribute>
						</xsl:if>
						<!-- SECTION -->
				...
				<div class="visible-print">
					<p>
							<b><xsl:text>Date Posted:&#160;</xsl:text></b>
				...
				<!-- CONTENTSBLOCK PRINT ONLY -->
				<xsl:if test="count(//janes:title[not(ancestor::janes:noteGroup)]) &gt; 4">
						<div class="visible-print printContents">
								<div>
						...
						<xsl:if test="$meta/jm:equipment[@mode = 'baseModel']/jm:standardName!=''">
								...
								<div class="panel panel-default u-overflow-hidden">
										...
										<table class="table table-bordered">
												<caption class="sr-only">Equipment Details</caption>
												<thead>
														<tr>
				...
				<xsl:if test="$meta/jm:classification[@taxonomy = 'Section'][1]/jm:standardName = 'Upcoming'">
						<div class="well width-30">
								<xsl:for-each select="$meta/dc:subject[@qualifier = 'seminar']|$meta/dc:subject[@qualifier = 'webex']">
		...
		<xsl:template match="janes:sect1[janes:title/@qualifier='rating']">
				<div class="u-overflow-hidden">
						<h2><xsl:value-of select="janes:title"/></h2>
						<xsl:choose>
				...
				<xsl:choose>
						...
						<xsl:when test="janes:title='Summary' and janes:sect2/@qualifier='box' and $level = 'sect1'">
								<div class="panel panel-default u-overflow-hidden">
										...
										<table class="table table-bordered">
											<xsl:if test="janes:title">
												<caption class="sr-only"><xsl:apply-templates select="janes:title" mode="very_plain" /></caption>
											</xsl:if>
		...
		<xsl:template match="janes:fleetList">
			...
			<xsl:choose>
			<xsl:when test="//janes:shipInstance//janes:outOfService[@label and not(@label = '')]"><xsl:value-of select="1"/></xsl:when>
						...
						<xsl:text>thumbnail-container u-overflow-hidden</xsl:text>
						 <xsl:choose>
							 <xsl:when test="janes:mediaObject[@display= 'inline']">
								 <xsl:choose> 
								 <xsl:when test="janes:mediaObject/@thumbnailSize = 'small' and janes:mediaObject/@align = 'center'"><xsl:text> width-20</xsl:text></xsl:when>
								 <xsl:when test="janes:mediaObject/@thumbnailSize = 'small'"><xsl:text> width-20 u-margin-Rl</xsl:text></xsl:when>
								 <xsl:when test="janes:mediaObject/@thumbnailSize = 'medium' and janes:mediaObject/@align = 'center'"><xsl:text> width-30</xsl:text></xsl:when> 
								 <xsl:when test="janes:mediaObject/@thumbnailSize = 'medium'"><xsl:text> width-30 u-margin-Rl</xsl:text></xsl:when>  
								 <xsl:when test="janes:mediaObject/@thumbnailSize = 'large'"><xsl:text> width-60</xsl:text></xsl:when>
								 <xsl:otherwise>
				...
				<xsl:choose>
                      ...
                      <xsl:attribute name="href">
                        <xsl:value-of select="concat('#', $hrefid)" />
											...
											<xsl:attribute name="data-refnote">
													<xsl:value-of select="concat('#', $hrefid)" />
											</xsl:attribute>
