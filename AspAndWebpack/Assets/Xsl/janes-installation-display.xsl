...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:janes="http://dtd.janes.com/2002/Content/">
  ...
  <xsl:param name="stringLookups">
    ...
    <string language="en" key="featureTypes">Feature Types</string>
    <string language="en" key="runways">Runways</string>
    <!-- PUT IN ALPHABETICAL ORDER PLEASE -->
  </xsl:param>
  ...
  <xsl:template match="/">
    <h1 class="page-header u-margin-Bm u-margin-Ts u-padding-Bs">
      <xsl:value-of select="concat(/installation/title, ' - ',/installation/installationType, ' ', /installation/subType, ' in ', /installation/country)"/>
    </h1>
    <xsl:apply-templates />
  </xsl:template>
  ...
  <xsl:template match="installation/confidence">
    <xsl:element name="tr">
      <xsl:call-template name="table_label">
