...
namespace IHS.Apps.CMP.Test.Utilities.Parsers
{
    ...
    public class QueryStringParserTest
    {
        ...
        /// <summary>Tests the advanced search mixed 'title' and 'full text' indexers.</summary>
        [TestMethod]
        public void TestAdvancedSearchMultiMixedIndexers()
        {
            var search = SetUpSearch();
            var qsp = new QueryStringParser(new ParserConfigObject() { CurrentProvider = DataProviderManager.GetProvider("JanesFast") });
            qsp.Parse("[apple]<or>[title|teaser(banana)]", search);

            var mainConstraint = search.Constraints[0] as OrNestedConstraint;
            Assert.IsTrue(mainConstraint.Count == 2);

            var secondNested = mainConstraint[1] as OrNestedConstraint;
            Assert.IsTrue(secondNested.Count == 2);

            var constr2 = secondNested[0] as IndexedSearchBehaviorConstraint;
            Assert.AreEqual("teaser", constr2.Indexer.ObjectKey);
            Assert.AreEqual("banana", (constr2.Constraint as SearchBehaviorConstraint).SearchTextValue);

            var constr = secondNested[1] as IndexedSearchBehaviorConstraint;
            Assert.AreEqual("title", constr.Indexer.ObjectKey);
            Assert.AreEqual("banana", (constr.Constraint as SearchBehaviorConstraint).SearchTextValue);
        }
        ...
        public void TestFilterColumnsQueryAdvancedSearch()
        {
            ...
            var mainConstraint = search.Constraints[0] as NestedConstraint;
            Assert.IsTrue(mainConstraint.Count == 4);
                        
            var value = (mainConstraint[0] as SearchBehaviorConstraint).SearchTextValue;
        ...
        public void TestFilterFullTextQueryAdvancedSearch()
        {
            ...
            var value = (search.Constraints[0] as SearchBehaviorConstraint).SearchTextValue;
        ...
        public void TestFilterFullTextWithCollumnsQueryAdvancedSearch()
        {
            ...
            var search = SetUpSearch();
            var qsp = new QueryStringParser(new ParserConfigObject() { CurrentProvider = DataProviderManager.GetProvider("JanesFast") });
            qsp.Parse("[FullText(Mark-44)]<and>[ViewItemDescription|ViewPlatformFamily|ViewItemName|ViewSourcesData(FoxTrot-99)]", search);

            var mainConstraint = search.Constraints[0] as NestedConstraint;
            Assert.IsTrue(mainConstraint.Count == 2);

            var valueFullText = (mainConstraint[0] as SearchBehaviorConstraint).SearchTextValue;
            Assert.AreEqual("Mark-44", valueFullText);

            var orNestedConstraint = mainConstraint[1] as OrNestedConstraint;
            Assert.IsTrue(orNestedConstraint.Count == 4);
            
            var valueEachColumn = (orNestedConstraint[0] as SearchBehaviorConstraint).SearchTextValue;
            Assert.AreEqual("FoxTrot-99", valueEachColumn);
        }

        [TestMethod]
        public void TestFilterFullTextWithOneCollumnAdvancedSearch()
        {
            //Arrange
            var search = SetUpSearch();
            var qsp = new QueryStringParser(new ParserConfigObject() { CurrentProvider = DataProviderManager.GetProvider("JanesFast") });
            qsp.Parse("[FullText(Mark-44)]<and>[ViewSourcesData(J S F)]", search);
            ...
            var valueColumn = (mainConstraint[1] as SearchBehaviorConstraint).SearchTextValue;
            Assert.AreEqual("J S F", valueColumn);
        }


        [TestMethod]
        public void TestFilterFullTextWithCollumnsWordsSpacesAdvancedSearch()
        {
            //Arrange
            var search = SetUpSearch();
            var qsp = new QueryStringParser(new ParserConfigObject() { CurrentProvider = DataProviderManager.GetProvider("JanesFast") });
            qsp.Parse("[FullText(Mark-44)]<and>[ViewItemDescription|ViewPlatformFamily|ViewItemName|ViewSourcesData(J S F)]", search);
            ...
            var valueEachColumn = (orNestedConstraint[0] as SearchBehaviorConstraint).SearchTextValue;
            Assert.AreEqual("J S F", valueEachColumn);
        }
        ...
        private static Search SetUpSearch()
        {
            ...
            var indexers = new ReadOnlyIndexerList(
                new CommonIndexerList<IIndexer>
                {
                    ...
                    new ProviderAttribute("teaser", "teaser", AttributeType.STRING_TYPE, null)
                });
