...
namespace IHS.Apps.CMP.Web.Controllers
{
    ...
    public class ChartPageController : CategoryController
    {
        ...
        private ChartPageModel GetResultsModel(bool getChart)
        {
            ...
            var facetParser = parserManager.GetParser<FacetParser>();
            if (facetParser != null)
            {
                var facets = facetParser.SimpleFacets;
                metaData.AppliedFacets = this.facetService.GetSelectedFacets(search, facets, configCategory);
            }
