    ...
    public abstract class AbstractSqlProvider : AbstractDataProvider
    {
        ...
        private string UpdateSqlTextSearchAllColumns(string sql, ICategory category, TSQLCategory tsqlCat, ISearch search, bool prefetching)
        {
            ...
            while (forEachUsedHintPresent)
            {
                ...
                if (!string.IsNullOrEmpty(newSql))
                {
                    ...
                    sql = AbstractSqlProvider.UpdateSqlConstraintClauses(sql, newSql, AbstractSqlProvider.WherePatternWithoutAuths);
                    sql = AbstractSqlProvider.UpdateSqlConstraintClauses(sql, newSql, AbstractSqlProvider.ConstraintsOnlyNoAuths);
        ...
        private string GetSql(ICategory category, string queryKey, ISearch search, AaaAuthorisation authorisations, bool isCountQuery)
        {
            ...
            if (!config.Query.IgnoreConstraints)
            {
                ...
                if (sqlExtra.ToString().Contains("contains("))
                {
                    TSQLCategory tsqlCat = this.GetCategoryConfig(category.Name);
            ...
            try
            {
                using (DbConnection connection = this.CreateDBConnection())
                {
                    using (DbCommand command = connection.CreateCommand())
                    {
                        ...
                        Log.Debug(Environment.NewLine + "Executing PreFetchData SQL *************" + Environment.NewLine + sql + Environment.NewLine + "********************");
