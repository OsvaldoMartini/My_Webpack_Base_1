function MapDrawing(map, layers) {
    ...
    function getExistingAreas(areasToJoin) {
        ...
        $.ajax('/MapArea/Get').done(function(result){
            ...
            result.forEach(function(area){
                self.existingAreas.push(new CustomArea(area.Id,area.Name, area.Feature, undefined, 'customarea'));
            });
    ...
    function getDefinedAreas() {
        ...
        if (self.definedAreas().length == 0) {
            $.get2('/MapArea/GetPredefined').done(function (result) {
                result.forEach(function (area) {
                    indexer = (area && area.areaType) ? 'customarea' : 'geoarea';
                    self.definedAreas.push(new CustomArea(area.Id, area.Name, area.Feature, indexer));
                });
            }).always(function () {
                self.isLoadingAreas(false);
            });
    ...
    function drawArea(theMap, customArea){
        ...
        var graphics = shapes.map(function(shape){
            ...
            graphic.areaType = customArea.areaType;
            layer.add(graphic);
            return graphic;
        });
        return graphics;
...
// Needd to update grpahic which areas are being search on edit
    ...
    self.search = function (area, areaToRemove) {
        if (!_mapPageVM.facetsVM.hasFacet("latlongs")) {
            _mapPageVM.updateMapExtentFacet(true);
        }

        var valuesOfCustomArea = _mapPageVM.facetsVM.getFacetValues("customarea");
        var valuesOfGeoArea = _mapPageVM.facetsVM.getFacetValues("geoarea");
        ...
        if (areaRemoved && typeof areaRemoved === "string") {
            indexer = (areaToRemove && areaToRemove.areaType) ? 'customarea' : 'geoarea';
            _mapPageVM.facetsVM.removeFacet(indexer, areaRemoved, undefined, true, operator);
            
            constraintsToRemove = { f: {} };
            constraintsToRemove.f[indexer] = areaRemoved;

            //check if needs to Re-Apply Facet with the <and> Operator when is alone
            if (_mapPageVM.facetsVM.hasFacet("customarea") || _mapPageVM.facetsVM.hasFacet("geoarea")) {
                if (!_mapPageVM.facetsVM.hasFacet("customarea")) {
                    //it needs to ReApply "geoarea" with <AND>
                    console.log("it needs to ReApply 'geoarea'");
                    
                }
                if (!_mapPageVM.facetsVM.hasFacet("geoarea")) {
                    //it needs to ReApply "customarea" with <AND>
                    console.log("it needs to ReApply 'customarea'");
        ...
        if (area) {
            indexer = (area && area.areaType) ? 'customarea' : 'geoarea';
            var operator = "<and>";
            if (_mapPageVM.facetsVM.hasFacet("customarea") && indexer == "geoarea") {
                operator = "<or>";
            } else if (_mapPageVM.facetsVM.hasFacet("geoarea") && indexer == "customarea")
                operator = "<or>";
            
            _mapPageVM.facetsVM.applyFacet(indexer, area.name, false, undefined, true, operator);
        } else {
...
function CustomArea(id, name, areas, systemShape, areaType){
    var self = this;
    self.id = id;
    self.name = name;
    self.areas = [];
