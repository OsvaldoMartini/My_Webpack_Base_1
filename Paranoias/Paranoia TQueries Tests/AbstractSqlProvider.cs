...
namespace IHS.Apps.CMP.DataProviders
{
    ...
    public abstract class AbstractSqlProvider : AbstractDataProvider
    {
        ...
        /// <summary>
        /// Holds the pattern to match when looking for join region hint.
        /// </summary>
        private const string JoinRegionPattern = "(JOIN_REGION)";

        /// <summary>
        /// Holds the pattern to match when looking for result table hint.
        /// </summary>
        private const string ResultTablePattern = "(RESULT_TABLE)";

        /// <summary>
        /// Holds the pattern to match when looking for region count hint.
        /// </summary>
        private const string RegionCountPattern = "(REGION_COUNT)";
            ...
            foreach (TSQLCategory tsqlConfig in categories)
            {
                try
                {
                    ...
                    for (int count = 0; count < categoryKeys.Length; count++)
                    {
                        ...
                        List<TSQLQuery> tsqlExtras = new List<TSQLQuery>();
                        ...
                        foreach (TSQLQuery query in tsqlConfig.TSQLQueries)
                        {
                            ...
                            if (!ignorePrefetch && query.RunInPreFetch)
                            {
                                ...
                                if (query.Name.Equals("JDBChart"))
                                {
                                    //Call twice for two queries cacche for JDBChart
                                    //Result Table
                                    query.Name = "JDBChart";
                                    var queryClone = (TSQLQuery)query.Clone();
                                    AbstractSqlProvider.SubstituteReplaceableResultTable(queryClone, tsqlConfig);
                                    AbstractSqlProvider.SubstituteReplaceableJoinRegionConditions(query, tsqlConfig);
                                    sql = queryClone.SqlCommand;
                                    tsqlExtras.Add(queryClone);
                                    this.PreFetchData(sql, providerCategory, indexers, config.MetaData);

                                    query.Name = "JDB_REGION";
                                    AbstractSqlProvider.SubstituteReplaceableRegionCount(query, tsqlConfig);
                                    AbstractSqlProvider.SubstituteReplaceableJoinRegionConditions(query, tsqlConfig);
                        ...
                        if (tsqlExtras.Count > 0)
                        {
                            foreach(TSQLQuery queryExtra in tsqlExtras)
                            {
                                tsqlConfig.TSQLQueries.Add(queryExtra);
                            }
                        }
        ...
        /// <summary>
        /// Replace a result table hint in the SQL command with a category level result table
        /// element text - reduces duplication.
        /// </summary>
        /// <param name="query">Query to edit.</param>
        /// <param name="tsqlConfig">Category to use to get the result table.</param>
        private static void SubstituteReplaceableResultTable(TSQLQuery query, TSQLCategory tsqlConfig)
        {
            query.SqlCommand = AbstractSqlProvider.ValidatedReplace(
                query.SqlCommand,
                Regex.Escape(AbstractSqlProvider.ResultTablePattern),
                string.IsNullOrEmpty(tsqlConfig.ResultTable) ? string.Empty : tsqlConfig.ResultTable);
        }

        /// <summary>
        /// Replace a region count hint in the SQL command with a category level region count
        /// element text - reduces duplication.
        /// </summary>
        /// <param name="query">Query to edit.</param>
        /// <param name="tsqlConfig">Category to use to get the region count.</param>
        private static void SubstituteReplaceableRegionCount(TSQLQuery query, TSQLCategory tsqlConfig)
        {
            query.SqlCommand = AbstractSqlProvider.ValidatedReplace(
                query.SqlCommand,
                Regex.Escape(AbstractSqlProvider.RegionCountPattern),
                string.IsNullOrEmpty(tsqlConfig.RegionCount) ? string.Empty : tsqlConfig.RegionCount);
        }

        /// Replace a join region hint in the SQL command with a category level join region
        /// element text - reduces duplication.
        /// </summary>
        /// <param name="query">Query to edit.</param>
        /// <param name="tsqlConfig">Category to use to get the join region.</param>
        private static void SubstituteReplaceableJoinRegionConditions(TSQLQuery query, TSQLCategory tsqlConfig)
        {
            query.SqlCommand = AbstractSqlProvider.ValidatedReplace(
                query.SqlCommand,
                Regex.Escape(AbstractSqlProvider.JoinRegionPattern),
                string.IsNullOrEmpty(tsqlConfig.JoinRegion) ? string.Empty : tsqlConfig.JoinRegion);
        }
        ...
        /// <param name="query">Query to edit.</param>
        /// <param name="tsqlConfig">Category to use to get the region count.</param>
        private static void SubstituteReplaceableIndexersSql(TSQLQuery query, TSQLCategory tsqlConfig)
        {
            query.SqlCommand = AbstractSqlProvider.ValidatedReplace(
                query.SqlCommand,
                Regex.Escape(AbstractSqlProvider.IndexersSqlPattern),
                string.IsNullOrEmpty(tsqlConfig.JoinRegion) ? string.Empty : tsqlConfig.JoinRegion);
        }
        ...
        private static string UpdateSqlForEachUsedHints(string sql, ISearch search, bool prefetching)
        {
            NestedConstraint nestedConstraint = null;
            if (search != null && search.Constraints != null)
            {
                ...
                else
                {
                    ...
                    foreach (var ixr in search.SearchSource.Indexers)
                    {
                        if (ixr.ObjectKey.StartsWith(bits[1], StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            if (isUsed)
                            {
                                newSql += ReplaceAttributeSuffix(bits[1], bits[2], ixr.ObjectKey);
                            }
                        }
                    }
                }
        ...
        /// <param name="prefetching">Flag to indicate we are prefetching(loading attributes).</param>
        /// <returns>Corrected sql.</returns>
        private string UpdateSqlTextSearchAllColumns(string sql, ICategory category, TSQLCategory tsqlCat, ISearch search, bool prefetching)
        {
            NestedConstraint nestedConstraint = null;
            if (search != null && search.Constraints != null)
            {
            ...
            const string ForEachUsedHint = "|(TEXTSEARCHALLCOLUMNS::";
            const string RightBracketAndPipe = ")|";
            bool isUsed = false;
            bool forEachUsedHintPresent = false;
            string newSql = string.Empty;
            ...
            try
            {
                using (DbConnection conn = this.CreateDBConnection())
                {
                    using (DbCommand objCmd = conn.CreateCommand())
                    {
                        ...
                        objCmd.CommandType = CommandType.Text;
                        objCmd.CommandText = sql;
                        Log.Debug(Environment.NewLine + "Executing SQL *************" + Environment.NewLine + sql + Environment.NewLine + "********************");
                        objCmd.CommandTimeout = 1000;
        ...
        private string GetSql(ICategory category, string queryKey, ISearch search, AaaAuthorisation authorisations, bool isCountQuery)
        {
            ...
            if (!config.Query.IgnoreConstraints)
            {
                ...
                sql = AbstractSqlProvider.RemoveHint(sql, AbstractSqlProvider.RegionCountPattern);
                sql = AbstractSqlProvider.RemoveHint(sql, AbstractSqlProvider.ResultTablePattern);
                sql = AbstractSqlProvider.RemoveHint(sql, AbstractSqlProvider.JoinRegionPattern);
